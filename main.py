...
from shared.ai_client        import AIConfig, detect_best_ollama_model, get_ollama_models
...
from backend.generate_docs   import generate_all_docs, ProgressTracker
from backend.validate_backend import (
    validate_backend, save_validation_report, print_validation_summary,
)
...
def write_pages_as_files(pages, docs_frontend_dir, config, no_ai, append=False):
    from frontend.generate_docs import _skeleton_page
    from shared.ai_client import call_ai
    from prompts.frontend_prompts import pages_md_system, pages_md_prompt

    os.makedirs(docs_frontend_dir, exist_ok=True)
    sys_msg  = pages_md_system()
    new_rows = []

    for i, page in enumerate(pages):
        route     = page.get("path", "unknown")
        component = page.get("component", "UNKNOWN")
        api_calls = page.get("api_calls", [])

        safe_name = route.strip("/").replace("/", "_").replace("-", "_") or "home"
        safe_name = re.sub(r"[^\w]", "_", safe_name).strip("_") or "home"
        out_file  = os.path.join(docs_frontend_dir, safe_name + ".md")

        print("    [{}/{}] {} -> {}.md".format(i + 1, len(pages), route, safe_name))

        if no_ai or not config.use_ai:
            content = _skeleton_page(page)
        else:
            prompt  = pages_md_prompt(page)
            content = call_ai(prompt, config, system=sys_msg, max_tokens=1200)
        ...
        new_rows.append("| `{}` | `{}` | {} | [{}]({}) |\n".format(
            route, component, len(api_calls),
            safe_name + ".md", safe_name + ".md",
        ))

    index_path = os.path.join(docs_frontend_dir, "index.md")
    if append and os.path.exists(index_path):
        ...
        with open(index_path, "w", encoding="utf-8") as f:
            f.write("# Frontend Pages Index\n\n")
            f.write("| Route | Component | API Calls | File |\n")
            f.write("|-------|-----------|-----------|------|\n")
            for row in new_rows:
                f.write(row)
            f.write("\n---\n_Generated by doc_writer_\n")

    print("  {} page files written".format(len(pages)))
...
def cmd_generate_docs(args):
    ...
    def run_frontend():
        ...
        already_paged = _get_already_paged(docs_frontend)
        pending_pages = [
            p for p in pages
            if _page_filename(p.get("path", "")) not in already_paged
        ]

        if already_paged and not args.force:
            print("\n  [RESUME] {}/{} pages already documented".format(
                len(already_paged), len(pages)))
            if not pending_pages:
                print("  All pages done. Use --force to regenerate.")
                return
            pages_to_doc = pending_pages
        else:
            pages_to_doc = pages

        print("\n  Step 2: Writing one .md file per page...")
        write_pages_as_files(
            pages_to_doc, docs_frontend, config, no_ai,
            append=(bool(already_paged) and not args.force),
        )
